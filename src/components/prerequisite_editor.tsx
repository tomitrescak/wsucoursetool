...
import { State, Prerequisite, Unit, AcsKnowledge } from './types';
...
const PrerequisiteLine = ({ prerequisite, acsSkills, state, owner, i }) => (
...
const AddPrerequisite = observer(({ unit, state, acsSkills, owner }) => {
...
const OrEditor = observer(({ owner, prerequisite, acsSkills, state, unit }: OrEditorProps) => (
  <Pane display="flex" marginBottom={8}>
    ...
    <Pane flex="1" marginLeft={8}>
      {prerequisite.prerequisites.map((p, i) => (
        <Pane key={i}>
          <Prerequisites owner={p} acsSkills={acsSkills} state={state} unit={unit} />
...
const Prerequisites = observer(({ owner, acsSkills, state, unit }) => {
  return (
    <Pane>
      ...
      <AddPrerequisite acsSkills={acsSkills} owner={owner} state={state} unit={unit} />
    </Pane>
  );
});

export const PrerequisiteEditor: React.FC<Props> = observer(({ state, owner, unit }) => {
  ...
  return (
    <Pane flex={1}>
      ...
      {expanded && (
        <Pane marginTop={8}>
          <Prerequisites acsSkills={acsSkills} owner={owner} unit={unit} state={state} />{' '}
        </Pane>
      )}
    </Pane>
  );
